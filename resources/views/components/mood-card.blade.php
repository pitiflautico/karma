...
<div class="flex items-center justify-between">
    ...
    <div class="flex items-center space-x-3 flex-1">
        ...
        <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
                <h4 class="font-bold text-[#292524] text-[18px]">{{ $mood->mood_name }}</h4>
            ...
            @if($mood->note)
                <p class="text-base font-normal text-[#57534e] line-clamp-2">{{ Str::words($mood->note, 14, '...') }}</p>
            @elseif($mood->calendarEvent)
                <p class="text-base font-normal text-[#57534e] line-clamp-2">{{ Str::words($mood->calendarEvent->title, 14, '...') }}</p>
            @else
                <p class="text-base font-normal text-[#a8a29e] italic">No notes</p>
            @endif
            ...
            @if($mood->needsDoctorConsultation())
                <div class="flex items-center mt-2 text-[#d97706] text-sm gap-1">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
                    </svg>
                    <span class="font-medium">Consult with your doctor.</span>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="flex items-center space-x-3 ml-3 flex-shrink-0">
        <span class="text-base font-normal text-[#57534e] flex-shrink-0">{{ $mood->created_at->format('g:iA') }}</span>
        <button
            type="button"
